...
uid = info['userId']
post_data = {"tag":"student@stu_infor_table","params":{"studId":"%s" % uid}}
post_url = 'http://uims.jlu.edu.cn/ntms/service/res.do'
r = s.post(post_url, data=json.dumps(post_data), headers=headers)
info = json.loads(r.text)['value']
schId = info['school']['schId']
deptId = info['department']['deptId']
egrade = info['egrade']
adcId = info['adminClass']['adcId']
